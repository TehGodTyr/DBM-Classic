...
L.DYNAMIC_MULTIPLE			= "DBM отключил несколько таймеров и предупреждений на этом боссе, т.к. нет точной информации о том, как работают механики боя для рейда вашего размера."
...
L.RAID_DOWN             = "%s пройден спустя %s!"
L.RAID_DOWN_L           = "%s пройден спустя %s! Ваше самое быстрое прохождение %s."
L.RAID_DOWN_NR            = "%s пройден спустя %s! Это новый рекорд! (Старый рекорд был %s)."
L.GUILD_BOSS_DOWN			= "Гильдия победила %s спустя %s!"
L.COMBAT_ENDED_AT			= "Бой с %s (%s) закончился спустя %s."
L.COMBAT_ENDED_AT_LONG		= "Бой с %s (%s) закончился спустя %s. На этом уровне сложности вы потерпели поражение уже %d раз."
L.GUILD_COMBAT_ENDED_AT		= "Гильдия потерпела поражение на %s (%s) спустя %s."
...
L.COMBAT_STARTED_AI_TIMER	= "My CPU is a neural net processor; a learning computer. (This fight will use the new timer AI feature to generate timer approximations)"-- Translate
...
L.NOTESHAREERRORGROUPFINDER	= "Нельзя поделиться заметкой на поле боя, в поиске рейда или подземелья"
...
L.OPTION_CATEGORY_DROPDOWNS		= "Dropdowns"--Still put in MISC sub grooup, just used for line separators since multiple of these on a fight (or even having on of these at all) is rare. -- Translate
L.OPTION_CATEGORY_YELLS			= "Крики"
L.OPTION_CATEGORY_NAMEPLATES		= "Nameplates"-- Translate
...
L.WHISPER_COMBAT_END_WIPE_STATS_AT	= "%s потерпел поражение от %s на %s. Общее количество поражений у них - %d."
...
L.VOICE_COUNT_MISSING		= "Голос отсчета %d использует голосовой пакет, который не был найден. Он был сброшен на настройки по умолчанию."
--L.BIG_WIGS						= "BigWigs"
...
L.UPDATEREMINDER_HOTFIX			= "Ваша версия DBM имеет известные проблемы с этим боссом, которые исправлены, если вы обновитесь до последней версии"
L.UPDATEREMINDER_HOTFIX_ALPHA	= "Ваша версия DBM имеет известные проблемы с этим боссом, которые исправлены в предстоящей версии (или последней альфа-версии)"
...
L.OUTDATEDSPELLTIMERS       = "ПРЕДУПРЕЖДЕНИЕ: DBM-SpellTimers ломает DBM и должен быть отключен для правильного функционирования DBM."
L.OUTDATEDRLT           = "ПРЕДУПРЕЖДЕНИЕ: DBM-RaidLeadTools ломает DBM. DBM-RaidLeadTools больше не поддерживается и должен быть удален для правильного функционирования DBM."
L.VICTORYSOUND            = "ПРЕДУПРЕЖДЕНИЕ: DBM-VictorySound не совместим с этой версией DBM. Он должен быть удален до того как DBM продолжит работу, чтобы избежать конфликтов."
...
L.CLASSIC_ONLY            = "ПРЕДУПРЕЖДЕНИЕ: Эта версия DBM предназначена только для использования с WoW: Classic. Удалите эту версию и установить правильную версию DBM для вашей версии WoW."

L.MOVABLE_BAR				= "Передвинь меня!"
...
L.INFOFRAME_POWER			= "Power"-- Translate
L.INFOFRAME_AGGRO			= "Угроза"
L.INFOFRAME_MAIN				= "Main:"--Main power -- Translate
L.INFOFRAME_ALT				= "Alt:"--Alternate Power -- Translate
...
L.SLASHCMD_HELP2				= {
	...
	"/dbm lockout: Получить список текущих сохранений подземелий у членов рейда (псевдонимы: lockouts, ids) (требуются права лидера или помощника).",
...
L.TIME_TOO_SHORT          = "Таймер начала боя должен быть более 3 секунд."
...
L.ORB						  = "Сфера"
L.ORBS            = "Сферы"
L.CHEST						= "Сундук"--As in Treasure 'Chest'. Not Chest as in body part.
L.NO_DEBUFF				= "Не %s"--For use in places like info frame where you put "Not Spellname"
L.ALLY						= "Союзник"--Such as "Move to Ally"
L.ALLIES          = "Союзники"--Such as "Move to Allies"
...
L.EDGE				    = "Край комнаты"
L.FAR_AWAY				= "Далеко"
L.BREAK_LOS				= "Break LOS"-- Translate
L.RESTORE_LOS     = "Restore/Maintain LOS"-- Translate
L.SAFE						= "Безопасно"
L.NOTSAFE         = "Не безопасно"
L.SHIELD					= "Щит"
L.PILLAR          = "Колонна"
L.INCOMING				= "Прибытие %s"
L.BOSSTOGETHER    = "Боссы вместе"
L.BOSSAPART       = "Боссы раздельно"
--Common Locals end
...
L.AUTO_ANNOUNCE_TEXTS.countdown		= "%s через %%d сек"
...
L.AUTO_ANNOUNCE_OPTIONS.soon 			= "Предупреждать заранее о $spell:%s"
L.AUTO_ANNOUNCE_OPTIONS.sooncount		= "Предупреждать заранее о $spell:%s"
L.AUTO_ANNOUNCE_OPTIONS.countdown		= "Спамить заранее отсчёт до заклинания $spell:%s"
L.AUTO_ANNOUNCE_OPTIONS.prewarn 		= "Предупреждать заранее о $spell:%s"
...
L.AUTO_SPEC_WARN_OPTIONS.gtfo 			= "Спец-предупреждение выбегите из войд-зоны на земле"
...
L.AUTO_VOICE4_OPTION_TEXT			= "Звуковое оповещение о войд-зоне на земле"
...
L.AUTO_READY_CHECK_OPTION_TEXT	= "Проигрывать звук проверки готовности, когда начинают бой с боссом (даже если он не является целью)"
L.AUTO_SPEEDCLEAR_OPTION_TEXT   = "Показывать таймер для быстрого прохождения %s"
...
L.INSTANCE_INFO_DETAIL_DEBUG		= "Игрок: %s ТипРезультата: %s Подземелье: %s ID: %s Сложность: %d Размер: %d Прогресс: %s"
...
L.INSTANCE_INFO_RESULTS			= "Результаты сканирования сохранений. Обратите внимание, что подземелья могут появляться более одного раза, если в вашем рейде есть игроки с локализованными клиентами WoW."
L.INSTANCE_INFO_SHOW_RESULTS		= "Игроки, которые еще не ответили: %s\n|Hgarrmission:DBM:showRaidIdResults|h|cff3588ff[Показать текущие результаты]|r|h"
--L.INSTANCE_INFO_SHOW_RESULTS		= "Игроки, которые еще не ответили: %s"
...
L.SILENTMODE_IS                     = "SilentMode is "-- Translate

L.LDB_LOAD_MODS		= "Загрузить модули"

L.LDB_ENABLE_BOSS_MOD	= "Включить модуль"
